...
const SPECIAL_PLAYERS = require("./specialplayer.json");
...
const FIREBASE_WEB_CONFIG = {
    apiKey: process.env.FIREBASE_WEB_API_KEY || "AIzaSyBHGzgtUte321WMvTdO1LJLg59O2r73ER0",
    authDomain: process.env.FIREBASE_AUTH_DOMAIN || "auction-10874.firebaseapp.com",
    projectId: process.env.FIREBASE_PROJECT_ID || "auction-10874",
    databaseURL: "https://auction-10874-default-rtdb.asia-southeast1.firebasedatabase.app",
    storageBucket: process.env.FIREBASE_STORAGE_BUCKET || "auction-10874.firebasestorage.app",
    messagingSenderId: process.env.FIREBASE_MESSAGING_SENDER_ID || "1090377012454",
    appId: process.env.FIREBASE_APP_ID || "1:1090377012454:web:e6177a08fd29edd91bd727",
    measurementId: process.env.FIREBASE_MEASUREMENT_ID || "G-WJZL0KKBQ1",
};

function poolDisplayName(datasetId) {
...
app.get("/api/archives/:code", async (req, res) => {
    const code = String(req.params.code || "").toUpperCase();
    if (!code) return res.status(400).json({ ok: false, error: "invalid code" });
    let data = rooms[code];
    if (!data || !data.auctionEnded) {
        try {
            const snap = await db.ref(`archives/${code}`).once("value");
            if (snap.exists()) data = snap.val();
        } catch (e) {
            console.error("archive fetch", e.message);
        }
    }
    ...
    res.json({
        ...
        data: {
            squads: data.squads || {},
            purses: data.purse || data.purses || {},
            owners: data.teamOwners || getTeamOwners(data) || {},
            rules: data.rules || {},
            poolName: data.poolName || poolDisplayName(data.datasetId),
            hostName: data.hostName || data.adminUser || "",
            datasetId: data.datasetId || "ipl2026",
...
app.get("/api/user/games", async (req, res) => {
    ...
    try {
        const snap = await db.ref(`userGames/${decoded.uid}`).once("value");
        const val = snap.val() || {};
        const games = Object.entries(val)
            .map(([id, g]) => ({ id, ...g }))
            .sort((a, b) => (b.endedAt || 0) - (a.endedAt || 0));
        res.json({ ok: true, games, email: decoded.email || "", name: decoded.name || "" });
    } catch (e) {
...
app.post("/api/user/games", async (req, res) => {
    ...
    const gameKey = `${roomCode}_${decoded.uid}`;
    try {
        if (deviceId) {
            const dupSnap = await db.ref(`userGames/${decoded.uid}`).once("value");
            const existing = dupSnap.val() || {};
            const already = Object.values(existing).some(
                (g) => g && g.roomCode === roomCode && g.deviceId === deviceId
            );
            if (already) return res.json({ ok: true, skipped: true, reason: "duplicate_device_room" });
        }
        const payload = {
            roomCode,
            poolName: body.poolName || poolDisplayName(body.datasetId),
            hostName: body.hostName || "",
            displayName: body.displayName || decoded.name || "",
            team: body.team || null,
            datasetId: body.datasetId || "ipl2026",
            deviceId: deviceId || null,
            endedAt: body.endedAt || Date.now(),
        };
        await db.ref(`userGames/${decoded.uid}/${gameKey}`).set(payload);
        res.json({ ok: true });
    } catch (e) {
        res.status(500).json({ ok: false, error: e.message });
    }
...
const LANDING_SPA_PATHS = ["/features", "/how-it-works", "/pools", "/faq", "/room"];
LANDING_SPA_PATHS.forEach((p) => {
    app.get(p, (_req, res) => {
...
/* ================================================= */
/* 🖼️ STICKER UPLOADS (per-room, per-user)            */
/* ================================================= */
const STICKER_ROOT = path.join(__dirname, 'sticker_uploads');
try { fs.mkdirSync(STICKER_ROOT, { recursive: true }); } catch (_) {}
app.use('/sticker-uploads', express.static(STICKER_ROOT, {
    maxAge: '1h',
    fallthrough: true
}));

const sanitizeSegment = (s) => String(s || '').replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 40) || 'anon';

const stickerStorage = multer.diskStorage({
    destination: (req, file, cb) => {
        const room = sanitizeSegment(req.params.roomCode).toUpperCase();
        const user = sanitizeSegment(req.params.username);
        const dir = path.join(STICKER_ROOT, room, user);
        fs.mkdir(dir, { recursive: true }, (err) => cb(err, dir));
    },
    filename: (req, file, cb) => {
        const ext = (path.extname(file.originalname || '') || '.png').toLowerCase().slice(0, 6);
        const safeExt = /^\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(ext) ? ext : '.png';
        const base = `s_${Date.now()}_${Math.random().toString(36).slice(2, 8)}${safeExt}`;
        cb(null, base);
    }
});
const stickerUpload = multer({
    storage: stickerStorage,
    limits: { fileSize: 5 * 1024 * 1024, files: 60 }, // 5MB each, up to 60 files/batch
...
// Upload a batch of stickers for a user in a room
app.post('/api/stickers/:roomCode/:username',
    stickerUpload.array('stickers', 60),
    (req, res) => {
        const room = sanitizeSegment(req.params.roomCode).toUpperCase();
        const user = sanitizeSegment(req.params.username);
        const files = Array.isArray(req.files) ? req.files : [];
        const urls = files.map(f => `/sticker-uploads/${room}/${user}/${path.basename(f.filename)}`);
        res.json({ ok: true, urls });
    },
    // Multer error handler (keeps JSON response instead of HTML stack trace)
    (err, req, res, _next) => {
        console.error('sticker upload failed:', err && err.message);
        res.status(400).json({ ok: false, error: err.message || 'upload failed' });
    }
);

// List existing stickers for a user
app.get('/api/stickers/:roomCode/:username', (req, res) => {
    const room = sanitizeSegment(req.params.roomCode).toUpperCase();
    const user = sanitizeSegment(req.params.username);
    const dir = path.join(STICKER_ROOT, room, user);
    fs.readdir(dir, (err, names) => {
        if (err) return res.json({ ok: true, urls: [] });
        const urls = (names || [])
            .filter(n => /\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(n))
            .sort()
            .map(n => `/sticker-uploads/${room}/${user}/${n}`);
        res.json({ ok: true, urls });
    });
});

// Delete one sticker for a user
app.delete('/api/stickers/:roomCode/:username/:file', (req, res) => {
    const room = sanitizeSegment(req.params.roomCode).toUpperCase();
    const user = sanitizeSegment(req.params.username);
    const file = sanitizeSegment(req.params.file);
    const target = path.join(STICKER_ROOT, room, user, file);
    // Safety: target must still live under STICKER_ROOT
    if (!target.startsWith(STICKER_ROOT + path.sep)) return res.status(400).json({ ok: false });
    fs.unlink(target, () => res.json({ ok: true }));
});

// Remove all stickers for a room (called on auction end / room abandon)
function clearRoomStickers(roomCode) {
    const room = sanitizeSegment(roomCode).toUpperCase();
    const dir = path.join(STICKER_ROOT, room);
    if (!dir.startsWith(STICKER_ROOT + path.sep)) return;
    fs.rm(dir, { recursive: true, force: true }, (err) => {
        if (err) console.error(`⚠️ Could not clear stickers for ${room}:`, err.message);
        else console.log(`🧹 Cleared stickers for room ${room}`);
    });
}
...
// Archive a room snapshot for post-auction viewing even if
// the live room document gets removed later.
async function archiveRoom(roomCode) {
    const r = rooms[roomCode];
    if (!r) return;
    try {
        const payload = {
            ...
            teamOwners: getTeamOwners(r),
            rtmLeft: r.rtmLeft || {},
            adminUser: r.adminUser || null,
            hostName: r.adminUser || null,
            datasetId: r.datasetId || "ipl2026",
            poolName: poolDisplayName(r.datasetId),
        };
        await db.ref(`archives/${roomCode}`).set(payload);
        console.log(`📦 Firebase: Room ${roomCode} archived.`);
    } catch (e) {
        console.error("⚠️ Firebase Archive Failed:", e.message);
    }
}
...
const SPECIAL_PLAYER_SET = new Set((Array.isArray(SPECIAL_PLAYERS) ? SPECIAL_PLAYERS : []).map(normalizePlayerName));
    ...
    playerPool = PLAYERS;
        ...
        rooms[code] = {
            ...
            auction: {
                ...
                bid: 0, team: null, timer: 10, interval: null, lastBidTeam: null, specialIntroPlayer: null
            }
        };
...
// Fetch archived leaderboard from Firebase (works even after room is removed)
socket.on("getArchivedLeaderboard", async ({ roomCode } = {}) => {
    try {
        const code = String(roomCode || socket.room || "").toUpperCase();
        if (!code) return;

        // Prefer live room if present, else use archive
        let room = rooms[code];
        if (!room) {
            const liveSnap = await db.ref(`rooms/${code}`).once("value");
            if (liveSnap.exists()) room = liveSnap.val();
        }

        let playingXI = room && room.playingXI ? room.playingXI : null;
        let purse = room && room.purse ? room.purse : null;

        if (!playingXI) {
            const archSnap = await db.ref(`archives/${code}`).once("value");
            if (archSnap.exists()) {
                const arch = archSnap.val();
                playingXI = arch.playingXI || {};
                purse = arch.purse || {};
            }
        }

        const teamNames = Object.keys(purse || playingXI || {});
        const boardMap = {};
        teamNames.forEach(t => {
            const entry = playingXI ? playingXI[t] : null;
            const p = purse && purse[t] != null ? purse[t] : 0;
            if (entry) {
                boardMap[t] = { ...entry, team: t, rating: entry.disqualified ? 0 : (entry.rating != null ? entry.rating : 0), purse: p };
            } else {
                boardMap[t] = { team: t, rating: 0, disqualified: true, reason: "No XI submitted", purse: p, xi: [] };
            }
        });
        ...
        console.error("getArchivedLeaderboard failed:", e.message);
    }
});
    ...
    socket.on("joinRoom", ({ roomCode, user }) => {
        ...
        if (existingSocketId) {
            ...
            // Also emit legacy payload used by some clients/popups
            io.to(oldSocketId).emit("identityChallenge", {
                code,
                name: user,
                roomCode,
                expiresIn: 30
            ...
            setTimeout(() => {
                ...
                if (ch && ch.newSocketId === socket.id) {
                    io.to(ch.oldSocketId).emit("identityDismiss");
 ...
 socket.on("verifyIdentityCode", ({ roomCode, name, code }) => {
    ...
    io.to(newSocketId).emit("identitySuccess", { message: "Verification successful. Joined with this name." });
    io.to(oldSocketId).emit("identityTakeoverSuccess", { by: name });
});
    ...
    socket.on("setRules", rules => {
        ...
        const bidTimer = Number(incoming.bidTimer);

        if(room.auctionStarted) {
            const patch = {};
            if ([5, 7, 10, 15].includes(bidTimer)) patch.bidTimer = bidTimer;
            if (incoming.minSquadSize != null) {
                const minSq = Number(incoming.minSquadSize);
                if (!Number.isNaN(minSq) && minSq >= 1 && minSq <= 30) patch.minSquadSize = minSq;
            }
            if (Object.keys(patch).length === 0) return;
            room.rules = { ...room.rules, ...patch };
            if (patch.bidTimer != null && room.auction && room.auction.live) {
                room.auction.timer = patch.bidTimer;
            }
        ...
        room.rules = { ...room.rules, ...incoming };
        if (![5, 7, 10, 15].includes(Number(room.rules.bidTimer))) room.rules.bidTimer = 10;
    ...
    socket.on("specialIntroFinished", ({ playerName }) => {
        const r = rooms[socket.room];
        if (!r || r.admin !== socket.id) return;
        if (!r.auction || !r.auction.player) return;
        const current = normalizePlayerName(r.auction.player.name);
        const incoming = normalizePlayerName(playerName);
        if (incoming && current !== incoming) return;

        if (r.auction.paused && r.auction.specialIntroPlayer) {
            r.auction.paused = false;
            r.auction.specialIntroPlayer = null;
            io.to(socket.room).emit("auctionResumed");
            sendLog(r, socket.room, "▶ Auction resumed after special intro");
            syncRoom(socket.room);
        }
    });
    ...
    socket.on("bid", () => {
        ...
        r.auction.timer = Number(r.rules.bidTimer) || 10;
    ...
    socket.on("rtmAccept", ({ amount }) => {
        ...
        if (buyerSocketId) {
            ...
            r.auction.rtmPending = null;
            r.rtmLeft[pend.pteam] = Math.max(0, (r.rtmLeft[pend.pteam] || 0) - 1);
                    ...
                    hostLeftEmptyTimers[roomCode] = setTimeout(() => {
                        ...
                        if (activeCount === 0) {
                            ...
                            if (!r.auctionEnded) {
                                ...
                                // 3. Remove any sticker uploads for this room
                                clearRoomStickers(roomCode);
                            } else {
...
function nextPlayer(r, room) {
    ...
    const isSpecialPlayer = SPECIAL_PLAYER_SET.has(normalizePlayerName(r.auction.player.name));
    if (isSpecialPlayer) {
        r.auction.paused = true;
        r.auction.specialIntroPlayer = r.auction.player.name;
    }
    ...
    if (isSpecialPlayer) {
        io.to(room).emit("specialPlayerIntro", {
            playerName: r.auction.player.name,
            videoPath: `/special player vid/${encodeURIComponent(normalizePlayerName(r.auction.player.name))}.mp4`
        });
        io.to(room).emit("auctionPaused");
    }
...
function endAuction(r, room) {
    ...
    // Persist live state and also archive a snapshot for later viewing
    syncRoom(room);
    archiveRoom(room);

    // Cleanup uploaded stickers for this room (per product spec: delete when auction ends)
    clearRoomStickers(room);
}
...
// Secondary listener so Emergent's k8s ingress (which routes `/api/*` to port 8001)
// reaches the same Express app for sticker/room APIs. Socket.io stays on the primary.
const SECONDARY_PORT = Number(process.env.SECONDARY_PORT) || 8001;
if (SECONDARY_PORT && SECONDARY_PORT !== PORT) {
    const http2 = require('http').createServer(app);
    http2.listen(SECONDARY_PORT, '0.0.0.0', () => {
        console.log(`Secondary HTTP listener on http://0.0.0.0:${SECONDARY_PORT}`);
    }).on('error', (e) => console.error('Secondary listener failed:', e.message));
}
